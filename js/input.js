...
function updatePointerFromEvent(event) {
  ...
  Input.cursorX = event.clientX - rect.left;
  Input.cursorY = event.clientY - rect.top;
}
...
function setKey(key, isDown) {
  ...
  if (isDown && key.length === 1) {
    ...
    if (Input.secretBuffer === CONFIG.SECRET_WORD && Game && Game.mode === "playing") Game.unlockSecret();
  }
}
    ...
    fullscreenButton.addEventListener("click", function () {
      ...
      if (!document.fullscreenElement) {
        if (!target.requestFullscreen) {
          if (Game && Game.showMessage) Game.showMessage("FULLSCREEN IS NOT SUPPORTED BY THIS BROWSER");
          return;
        }
        target.requestFullscreen().catch(function () {
          if (Game && Game.showMessage) Game.showMessage("FULLSCREEN REQUEST BLOCKED");
        });
