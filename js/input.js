...
function bindTouch() {
  document.querySelectorAll("[data-key]").forEach(function (button) {
    ...
    button.addEventListener("pointerdown", function (event) { event.preventDefault(); if (button.setPointerCapture) button.setPointerCapture(event.pointerId); if (key === "jump" && !Input.jump) Input.jumpPressed=true; Input[key]=true; });
    ["pointerup","pointercancel","pointerleave"].forEach(function (name) { button.addEventListener(name, function () { Input[key]=false; }); });
  });
  var fire=document.getElementById("fireButton"); if (fire) fire.addEventListener("pointerdown", function (event) { event.preventDefault(); Game.fire(); });
  var exit=document.getElementById("exitSecret"); if (exit) exit.addEventListener("click", function () { Game.exitSecret(); });
  var canvas=document.getElementById("game"); if (canvas) canvas.addEventListener("pointerdown", function () { Game.fire(); });
  var fullscreen=document.getElementById("fullscreenButton");
  if (fullscreen) fullscreen.addEventListener("click", function () { var target=document.querySelector(".game-wrap") || document.documentElement; if (!document.fullscreenElement) { if (!target.requestFullscreen) { Game.showMessage("FULLSCREEN IS NOT SUPPORTED BY THIS BROWSER"); return; } target.requestFullscreen().catch(function () { Game.showMessage("FULLSCREEN REQUEST BLOCKED"); }); } else if (document.exitFullscreen) document.exitFullscreen(); });
  document.addEventListener("fullscreenchange", function () { if (fullscreen) fullscreen.textContent=document.fullscreenElement ? "⛶ EXIT FULLSCREEN" : "⛶ FULLSCREEN"; });
}
