var Player = { x:0,y:0,vx:0,vy:0,onGround:false,angle:0,trail:[],health:3,hitUntil:0 };
Player.reset = function () { Player.x=Level.startX; Player.y=Level.startY-2; Player.vx=0; Player.vy=0; Player.onGround=false; Player.angle=0; Player.trail=[]; Player.health=3; Player.hitUntil=0; };
Player.update = function () {
  ...
  for(var j=0;j<Math.abs(Player.vy);j++){if(Collide.hitsSolid(Player.x,Player.y+sy,size,size)){if(sy>0)Player.onGround=true;Player.vy=0;break;}Player.y+=sy;}
